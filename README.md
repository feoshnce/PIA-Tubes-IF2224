...
## Deskripsi Program (Versi 2)
Pada Versi 2, program dikembangkan dengan penekanan pada peningkatan robustness, konsistensi internal, dan kualitas pengujian tanpa mengubah logika inti maupun perilaku input–output dari setiap milestone. Pada tahap analisis leksikal, ditambahkan mekanisme validasi konfigurasi DFA untuk memeriksa konsistensi state, transisi, dan kelas karakter serta mendeteksi potensi ambiguitas akibat overlapping character classes. Pada tahap sintaks dan semantik, diimplementasikan AST Contract Checker untuk memverifikasi konsistensi struktur AST pada batas parser dan semantic analyzer, serta Decorated AST Checker untuk memastikan anotasi semantik terpasang dengan benar setelah analisis semantik. Selain itu, ditambahkan fitur pelaporan konteks kesalahan (error context) yang bersifat opsional dan hanya aktif dalam mode debugging, sehingga membantu proses debugging tanpa memengaruhi keluaran standar program. Dari sisi pengujian, sistem test diperluas dengan penambahan kasus uji yang lebih kompleks seperti blok bersarang dan ekspresi majemuk, serta penyempurnaan test runner agar dapat mendeteksi dan menjalankan test secara otomatis. Seluruh pengembangan ini bersifat non-intrusive dan menjaga kompatibilitas penuh dengan hasil keluaran versi sebelumnya.
